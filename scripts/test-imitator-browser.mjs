...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    select.toggleCard("?");
    check(!select.choosingImitation && select.selectedCards.join() === "A", "Selected imitator should deselect without opening picker");
    ...
    const slotCount = reopened.cardSlotCount;
    reopened.cardSlotCount = reopened.selectedCards.length;
    reopened.toggleCard("?");
    check(!reopened.choosingImitation && reopened.selectedCards.join() === "A", "Full loadout must allow imitator deselection");
    check(JSON.parse(localStorage.getItem("characters-vs-geometry:last-card-loadout")).join() === "A", "Deselect must persist to last loadout");
    reopened.cardSlotCount = slotCount;
    reopened.toggleCard("?");
    check(reopened.choosingImitation, "Unselected imitator must still open picker");
    game.loop.wake();
