...
主菜单“回放”暂存本机最近三场战斗，支持观看、暂停、倍速、重播，以及 JSON 文件导入导出（上限 32 MB）。战斗结束或离开战场时记录，中途退出也保留已进行的部分；观看和导入不会覆盖最近三场或修改进度。

回放保存在独立的 IndexedDB 中，不随“存档”文件一起导出。续玩的无尽战场、联机重同步后的记录从对应快照开始，列表显示实际起止时间。回放需要兼容的游戏规则版本；旧战斗无法追溯补录。
