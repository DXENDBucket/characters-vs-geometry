...
| = | Special | Edge | 1000 | 30s | 0 | 0 | 0 | Unlocked after AE-3. Internal-edge connector; click cycles bidirectional, forward, backward and closed. No ammo storage or attackable body. | +25 shots/s flow per level; supports auto-upgrades |
...
| + | Function | Triangle | 1000 | 30s | 1200 | 150 | 0 | Processor: exactly five compatible shots become one, retaining separate judgments. Outputs finished shots first; results can be bundled again. | +25 capacity and +25 shots/s processing per effective level |
| - | Function | Triangle | 1000 | 30s | 1200 | 150 | 0 | Input-only interceptor. Local ammo cancels enemy bullets/mortars within radius 2.6, one shot per 0.1s. Spends 5 friendly damage per 1 enemy damage; retains leftovers. | 128 local capacity per effective level, like 0; upgrades retain stock |
| 0 | Function | Triangle | 600 | 10s | 1200 | 150 | 0 | Unlocked after AE-3. Buffers 128 real ordinary projectiles per effective level and automatically forwards through adjacent pipes. | +128 capacity per level; retains stock, supports auto-upgrades and extraction batches |
...
| 1 | Function | Triangle | 600 | 10s | 1200 | 150 | 0 | Unlocked after AE-3. Input-only outlet: waits for exactly n shots, then fires them together. Never forwards through pipes. Retains damage, hit counts, spread and range. | Manual number +1 raises required batch size, not damage; no auto-upgrades |
...
- `=` is a special edge connector (1000 cost, 30s cooldown). Deploy on an internal grid edge between adjacent logical cells. Click cycles `=`, `>`, `<`, closed; vertical links rotate the directions. No ammunition cache or attackable body. Flow replenishes at 25 shots/s per level, with up to one second of credit. Stack the selected = card to upgrade; auto-upgrade marks are supported. Erase via the same edge.
- Pipelines transfer between adjacent nodes, at most one edge per shot per simulation tick. Sources can feed 0, 1, + or - directly and must cost <=999. Only actual ordinary straight/spread/splash projectiles qualify; homing shots, lasers, mortars, skills and production bypass the circuit. Sources fire normally when all routes are closed, full or flow-limited. No copied actions or skipped SP/cooldown costs.
- Zero buffers 128 projectiles per effective level and automatically forwards them, rotating through available routes without duplication or immediate backtracking. Losing temporary levels retains excess stock but blocks input while full. Disconnected or blocked shots stay local; destruction loses stock.
- Numeric outlets are input-only: wait for exactly the displayed number of shots, then fire the whole batch together. Facing determines horizontal direction; spread, damage/type, source attribution, hit counts and remaining range are preserved. Higher numbers change grouping, not damage. Edge modes, upgrades, flow credit, local inventories and processor jobs persist in deterministic saves.
- 0 and 1 each cost 600 with 10s cooldown and retain the 1200 HP / 150 armor / 0 MR / 0 ATK panel. Stack 0 cards to upgrade bank capacity without losing inventory or turning into 1. Zero supports auto-upgrades and extraction batches. Outlet 1 still cannot auto-upgrade.
...
- Clear rewards: `+`, `-` and `&`.
- `+` has capacity 25 per effective level and processes 25 input shots/s per effective level. Exactly five compatible shots become one, preserving independent damage judgments and armor checks. Finished shots attempt downstream output first and can be bundled again; blocked stock stays local. Only numeric outlets fire into the battlefield. The + and - processors are exempt from the source cost limit.
