...
| = | Function | Triangle | 1000 | 30s | 1200 | 150 | 0 | Unlocked after AE-3. Links opposite horizontal/vertical neighbors. Numbers learn all sources costing <=999 in the entire connected equation. Expensive operands and adjacent operators break chains. | +1 level; highest valid equals level in the network sets the base imitation multiplier |
| + | Function | Triangle | 1000 | 30s | 1200 | 150 | 0 | Unlocked after AE-4. Ordinary source plus groups share learned-kind counts without extra source actions. Numeric + uses only its adjacent pair's sum; original numbers remain independent. Zero results store actions until clicked. | +1 own imitation multiplier, added to highest equals level minus 1; interval unchanged |
| - | Function | Triangle | 1000 | 30s | 1200 | 150 | 0 | Unlocked after AE-4. Numeric operands only: absolute difference of the adjacent pair. Operands remain independent. A zero result stores each kind until clicked. | Same operator multiplier rule as + |
| 0 | Function | Triangle | 100 | 2s | 1200 | 150 | 0 | Unlocked after AE-3; separately selectable number card. Stores each learned kind's actions, then clicks to imitate each once at its own stored count times the equation multiplier. Clears counts after release. | Number +1; starts at zero, first upgrade becomes 1; panel unchanged |
...
| 1 | Function | Triangle | 100 | 2s | 1200 | 150 | 0 | Unlocked after AE-3. Each learned kind separately counts actions by recorded sources. Every n actions, imitates at level n times the current network's highest equals level (default 1). Memories survive disconnection; unrelated same-kind towers never count. Explosions retain the number tower. | Increase displayed number by 1; own panel unchanged; temporary levels do not affect the number |
...
- Clear rewards: `=`, `1` and `0` (ASCII Expansion function towers). The old HP-bonus `+` has been replaced by the AE-4 equation operator.
- `=` costs 1000, cooldown 30s; `1` costs 100, cooldown 2s. Both use the standard 1200 HP, 150 armor, 0 MR, 0 ATK panel.
- Equals connects opposite neighbors horizontally and vertically. Every number learns all sources costing <=999 in the entire equation network, not only its neighbors: A=E=1 records both A and E; A=E=1=B=2 teaches both numbers A, E and B. Ordinary eligible towers bridge chains and branches; expensive operands and adjacent operators stop the connection. Cycles never duplicate a source. Numbers share memories but count independently by kind. Only recorded source towers count, not unrelated same-kind towers. Memories survive disconnection; deceased source identities are discarded without forgetting the behavior or accumulated count.
- Every n complete source actions triggers one imitation at level `n * E`, where E is the highest permanent level of a valid equals in the current connected equation (default 1). A level-2 equals makes number 9 act at level 18 every 9 actions. Upgrading operators does not reset action counts; breaking/moving connections recomputes E without erasing memories. Counts include attack cycles, skills, production and one-shot effects. A volley counts once, not per projectile. Numeric upgrades increase n instead of the physical panel and ignore temporary levels. Imitations never generate new counted actions.
- Imitation uses the number tower's position/facing and the copied action's level-scaled panel without replacing the tower. Explosions do not consume the number tower; other self-damage still applies. Permanent passive auras are not copied. Targeted skills keep the original target; box pushes keep the chosen direction. Memory, counts, skill durations and pending imitations are battle-save data.
- Zero is a separately selectable 100-cost, 2s-cooldown number tower. At number 0, each learned kind stores its own count and latest action payload. Click to imitate each stored kind once at `its count * current equation multiplier`, then clear all counts. Operator-generated zeros behave identically. Selecting the matching card still gives upgrading priority; a literal zero's first upgrade makes its number 1.
...
- Clear rewards: `+`, `-` and `&`.
- `+` shares counts within each continuous plus group, while `=` shares memories across the full equation. In `1=A+E+M+W=I`, an A action also advances E/M/W, but not I. Only the number towers imitate; source towers do not act again. `=+`, `++` and other adjacent operators are not operands and stop propagation. All operands must have base cost <=999.
- Ordinary chains such as `A+C+E` remain valid: one A action advances the learned A, C and E counters exactly once each. The shared increments do not propagate recursively or trigger actions by the physical C/E towers; C or E acting shares counts in the same way.
- Numeric arithmetic uses only an operator's adjacent pair: `3+5+2` makes the two pluses 8 and 7. `3-5` and `5-3` both make 2; `3-3` makes a storing zero. Horizontal and vertical channels work independently, with separate results, memories, counters and equals multipliers. A crossing operator alone does not merge systems; paths through shared operands can connect them. Clicking releases only zero-valued channels, leaving automatic channels untouched. Operands continue independently. Imitation level is `result * (E + own permanent level - 1)`; zero releases instead use each kind's stored count as the result. Minus costs 1000, cooldown 30s, and unlocks after AE-4. It cannot connect ordinary towers. Adjacent operators remain invalid.
