...
- Each bank holds 128 projectiles per effective level, including original hit counts. Overflow flies normally. Losing temporary levels retains excess stock but blocks capture until space is available. Shots can enter a circuit only once; multiple banks receive incoming shots in placement order without duplication. Clicking 0 discharges the stock at that click; incoming shots during discharge await the next click.
