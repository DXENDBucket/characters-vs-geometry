...
| = | Special | Edge | 100 | 4s | 0 | 0 | 0 | Unlocked after AE-3. Internal-edge connector; click cycles bidirectional, forward, backward and closed. No ammo storage or attackable body. | +25 shots/s flow per level; supports auto-upgrades |
...
| + | Function | Triangle | 100 | 4s | 1200 | 150 | 0 | Processor: exactly five compatible shots become one, retaining separate judgments. Outputs finished shots first; results can be bundled again. | +25 capacity and +25 shots/s processing per effective level |
| - | Function | Triangle | 100 | 4s | 1200 | 150 | 0 | Input-only interceptor. Local ammo cancels enemy bullets/mortars within radius 2.6, one shot per 0.1s. Spends 5 friendly damage per 1 enemy damage; retains leftovers. | 128 local capacity per effective level, like 0; upgrades retain stock |
| 0 | Function | Triangle | 100 | 4s | 1200 | 150 | 0 | Unlocked after AE-3. Buffers 128 real ordinary projectiles per effective level and automatically forwards through adjacent pipes. | +128 capacity per level; retains stock, supports auto-upgrades and extraction batches |
...
| 1 | Function | Triangle | 100 | 4s | 1200 | 150 | 0 | Unlocked after AE-3. Input-only outlet: waits for exactly n shots, then fires them together. Never forwards through pipes. Retains damage, hit counts, spread and range. | Manual number +1 raises required batch size, not damage; no auto-upgrades |
...
- Pipelines deliver to at most one subsequent actual node per shot per simulation tick; intervening transparent cells add no delay. Sources must cost <=999; the + and - processors are exempt. Only actual ordinary straight/spread/splash projectiles qualify; homing shots, lasers, mortars, skills and production bypass the circuit. Sources fire normally when all paths are closed, full or flow-limited. No copied actions or skipped SP/cooldown costs.
...
- All five pipeline components (=, 0, 1, +, -) cost 100 with 4s cooldown. The ordinary nodes retain the 1200 HP / 150 armor / 0 MR / 0 ATK panel. Stack 0 cards to upgrade bank capacity without losing inventory or turning into 1. Zero supports auto-upgrades and extraction batches. Outlet 1 still cannot auto-upgrade.
