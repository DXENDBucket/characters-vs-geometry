...
| = | Function | Triangle | 1000 | 30s | 1200 | 150 | 0 | Unlocked after AE-3. Links opposite horizontal/vertical neighbors. Numbers learn all sources costing <=999 in the entire connected equation. Expensive operands and adjacent operators break chains. | / |
| + | Function | Triangle | 1000 | 30s | 1200 | 150 | 0 | Unlocked after AE-4. Ordinary source plus groups share learned-kind counts without extra source actions. In numeric groups, every + acts as a number equal to the whole group's sum; original numbers retain their own behavior. Numbers cannot be added to ordinary towers. | / |
...
| 1 | Function | Triangle | 100 | 2s | 1200 | 150 | 0 | Unlocked after AE-3. Each learned kind separately counts actions by recorded sources. Every n actions, imitates once from its position at level n. Memories survive disconnection; unrelated same-kind towers never count. Explosions retain the number tower. | Increase displayed number by 1; own panel unchanged; temporary levels do not affect the number |
...
- Clear rewards: `=` and `1` (ASCII Expansion function towers). The old HP-bonus `+` has been replaced by the AE-4 equation operator.
...
- Every n complete source actions triggers one level-n imitation, including attack cycles, skills, production and one-shot effects. A volley counts as one action, not one per projectile. Numeric upgrades increase n instead of the physical panel and ignore temporary levels. Imitations never generate new counted actions.
...
- Numeric addition: in `A=3+5`, the `+` itself acts as number 8, imitating A every 8 actions at level 8; 3 and 5 still imitate independently at their own values. In `3+5+2`, both pluses use 10. Numbers and computed pluses share source memories across the whole equation, like `=` connections, but `=` never merges sums. Number/ordinary-tower addition is invalid. Upgrading a numeric operand recalculates the sum; temporary levels and the plus's own level do not affect it. The + glyph remains visible with its computed value below.
