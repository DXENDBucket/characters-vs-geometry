...
| 0 | Function | Triangle | 600 | 10s | 1200 | 150 | 0 | Unlocked after AE-3. Banks 128 real ordinary projectiles; click to discharge current stock through connected numeric outlets. Overflow passes normally. | Only a 1 card converts it into outlet 1, clearing stock; no auto-upgrades |
...
| 1 | Function | Triangle | 600 | 10s | 1200 | 150 | 0 | Unlocked after AE-3. Outlet for original banked shots: every 40ms emits up to n shots, with multiple outlets taking turns. Retains damage, multi-hit judgments, spread and remaining range. | Manual number +1 changes grouping, never total damage or shots; no auto-upgrades |
...
- `=` is a special edge connector (1000 cost, 30s cooldown). Deploy on an internal grid edge, connecting its two adjacent logical cells without being an attackable/blocking unit. Cannot be healed, mirrored, upgraded or shifted. Erase via the same edge. Mint links indicate a complete network.
- Arrange adjacent source/0/1 cells and link their shared edges. A circuit needs both a bank and an outlet before interception. Sources must cost <=999. Only actual ordinary straight/spread/splash projectiles qualify; homing shots, lasers, mortars, skills and production bypass the circuit. No copied actions or skipped SP/cooldown costs.
- Each bank holds 128 projectiles, including original hit counts. Overflow flies normally. Shots can enter a circuit only once; multiple banks receive incoming shots in placement order without duplication. Clicking 0 discharges the stock at that click; incoming shots during discharge await the next click.
- Every 40ms an outlet emits up to its displayed number of original shots, rotating between outlets. Its facing determines horizontal direction; spread, damage/type, source attribution and remaining range are preserved. Higher numbers change grouping, not total damage or shot count. Disconnection pauses release and retains stock; destruction loses a bank's stock. Edges, stored shots and pending discharge persist in deterministic battle saves. Old queued imitations are ignored.
- 0 and 1 each cost 600 with 10s cooldown and retain the 1200 HP / 150 armor / 0 MR / 0 ATK panel. Neither auto-upgrades, including old saved flags. Only a 1 card converts zero to outlet 1, clearing stock; stacking 0 cards cannot upgrade it. Zero placement/conversion bypass the extraction pool without consuming it.
...
- Clear rewards: `+`, `-` and `&`.
- `+` is a bundling outlet (up to five compatible shots per batch); `-` is an automatic interceptor (radius 2.6, one bank shot every 0.1s), including enemy mortars. These processors are exempt from the source cost limit. A zero bank plus either processor is a complete circuit without a numeric outlet.
- Cancellation consumes equal raw damage budgets from ammo and target. Multi-hit judgments are removed in order, with at most one partially damaged judgment. Leftover ammo stays in the bank; surviving enemy shots retain their reduced impact damage. Projectile size grows with damage budget and shrinks as it is consumed. Lasers and non-projectile skills bypass cancellation.
