...
| = | Function | Triangle | 1000 | 60s | 1200 | 150 | 0 | Unlocked after AE-3. Links opposite horizontal/vertical neighbors. Numbers learn all sources costing <=999 in the entire connected equation, including across ordinary towers. Chains share memories, not counters. | / |
...
- Clear rewards: `=` and `1` (ASCII Expansion function towers). The previous `+` remains removed.
- `=` costs 1000, cooldown 60s; `1` costs 100, cooldown 2s. Both use the standard 1200 HP, 150 armor, 0 MR, 0 ATK panel.
- Equals connects opposite neighbors horizontally and vertically. Every number learns all sources costing <=999 in the entire equation network, not only its neighbors: A=E=1 records both A and E; A=E=1=B=2 teaches both numbers A, E and B. Ordinary towers bridge chains and branches; expensive towers can bridge but cannot be learned. Cycles never duplicate a source. Numbers share memories but count independently by kind. Only recorded source towers count, not unrelated same-kind towers. Memories survive disconnection; deceased source identities are discarded without forgetting the behavior or accumulated count.
