...
import { damageEffectTextColor } from "../data/damageColors";
...
import type { TowerCombatRuntime as CardBehaviorRuntime, TowerReadinessRuntime as CardReadinessRuntime } from "./towerCombatRuntime";
...
interface BehaviorScratch {
  magicLaser: Enemy[]; healing: Tower[]; areaHealing: Tower[]; shift: Enemy[]; repel: Enemy[]; blocked: Enemy[];
}
const workspaces = new WeakMap<CardBehaviorRuntime, BehaviorScratch>();
function scratch(runtime: CardBehaviorRuntime) {
  let buffers = workspaces.get(runtime);
  if (!buffers) {
    buffers = { magicLaser: [], healing: [], areaHealing: [], shift: [], repel: [], blocked: [] };
    workspaces.set(runtime, buffers);
  }
  return buffers;
}
...
function fireHomingVolley(tower: Tower, definition: CardDefinition, runtime: CardBehaviorRuntime) {
  ...
  for (const muzzle of HOMING_PROJECTILE_MUZZLES) {
    runtime.projectiles.push(
      runtime.createHomingProjectile({
        x: tower.x + muzzle.x,
        y: tower.y + muzzle.y,
        lane: tower.lane,
...
function fireMagicLaser(tower: Tower, definition: CardDefinition, runtime: CardBehaviorRuntime) {
  ...
  const targets = magicLaserTargets(tower, runtime.enemies, direction, endX, scratch(runtime).magicLaser);
  const damage = towerAttackAmount(tower, definition);
  const damageType = towerDamageType(tower, definition.damageType ?? "magic", runtime.battleTime);

  runtime.presentation.laser(startX, tower.y, endX);
  try {
    for (const enemy of targets) {
      runtime.presentation.hit(enemy.x, enemy.y, damageType);
      ...
      if (enemy.inPlay && definition.projectileDebuff && definition.projectileDebuffDuration) {
        ...
        if (definition.projectileDebuff === "sunder") {
          runtime.presentation.sunder(enemy.x, enemy.y);
        }
      }
    }
  } finally {
...
function magicLaserTargets(tower: Tower, enemies: Enemy[], direction: number, endX: number, targets: Enemy[]) {
  targets.length = 0;
  for (const enemy of enemies) {
...
function fireHealingPulse(
  tower: Tower,
  definition: CardDefinition,
  runtime: CardBehaviorRuntime,
  hitCount = 1
) {
  const targets = getHealTargets(tower, definition, runtime.occupied, definition.healTargets ?? 1, scratch(runtime).healing);
  if (targets.length === 0) {
    return;
  }
  ...
  try {
    for (const target of targets) {
      repeatHits(hitCount, () => healTower(runtime, target, towerAttackAmount(tower, definition)));
    }
  } finally {
    targets.length = 0;
  }
...
function fireAreaHealingPulse(tower: Tower, definition: CardDefinition, runtime: CardBehaviorRuntime, hitCount = 1) {
  const targets = getAreaHealTargets(tower, runtime.towers, scratch(runtime).areaHealing);
  if (targets.length === 0) {
    return;
  }
  ...
  try {
    for (const target of targets) {
      repeatHits(hitCount, () => healTower(runtime, target, amount));
    }
  } finally {
    targets.length = 0;
  }
...
function getAreaHealTargets(tower: Tower, towers: Tower[], targets: Tower[]) {
  targets.length = 0;
...
function fireShiftPulse(tower: Tower, definition: CardDefinition, runtime: CardBehaviorRuntime) {
  const targets = getShiftTargets(tower, runtime.enemies, scratch(runtime).shift);
  if (targets.length === 0) {
    return;
  }
  ...
  try {
    for (const target of targets) {
      ...
      target.y = tower.y;
      runtime.presentation.enemyDepth(target, 60 + target.lane);
      syncPassengerPositionState(target);
      runtime.presentation.enemyPosition(target);
      runtime.presentation.shift(target.x, previousY, target.x, target.y);
    }
...
function fireLaneRepelPulse(tower: Tower, definition: CardDefinition, runtime: CardBehaviorRuntime) {
  ...
  const targets = getLaneRepelTargets(tower, runtime.enemies, scratch(runtime).repel);
  if (targets.length === 0) {
    return;
  }
...
function fireBlockedPushPulse(tower: Tower, definition: CardDefinition, runtime: CardBehaviorRuntime) {
  const targets = getBlockedEnemies(tower, runtime.towers, runtime.enemies, runtime.occupied, scratch(runtime).blocked);
  if (targets.length === 0) {
    return;
  }
  ...
  try {
    for (const target of targets) {
      ...
      syncPassengerPositionState(target);
      runtime.presentation.enemyPosition(target);
      runtime.presentation.shift(previousX, target.y, target.x, target.y);
    }
...
function fireSlash(tower: Tower, definition: CardDefinition, runtime: CardBehaviorRuntime, hitCount = 1) {
  ...
  if (target) {
    runtime.presentation.slash(target.x, target.y, damageType);
    repeatHits(hitCount, () => {
  ...
  runtime.presentation.slash(x, y, damageType);
  repeatHits(hitCount, () => {
...
function fireArcWave(tower: Tower, definition: CardDefinition, runtime: CardBehaviorRuntime) {
  ...
  const damageType = towerDamageType(tower, definition.damageType ?? "magic", runtime.battleTime);
  const direction = towerFacingDirection(tower);
  runtime.presentation.arc(tower.x - direction * CELL_WIDTH * 0.24, tower.y, damageType, direction);
...
function healTower(runtime: CardBehaviorRuntime, tower: Tower, amount: number) {
  if (changeTowerHealth(tower, amount, runtime.presentation.health) <= 0) {
    return;
  }

  runtime.presentation.heal(tower.x, tower.y);
}
