...
import { isNumberTower, isNumericOperatorType, numberTowerStates, towerFormType } from "./towerIdentity";
...
export function executeTowerImitation(tower: Tower, behavior: ImitationBehavior, event: TowerActionEvent, runtime: TowerImitationRuntime) {
  if (!tower.inPlay || !isNumberTower(tower) ||
    ...
    !numberTowerStates(tower).some(state => state.numberMemory?.some(entry => entry.type === behavior.type))) return;
