...
import type { CardDefinition, CardId, EdgeTower, Projectile, StoredTowerShot, Tower } from "../types";
...
interface Circuit { banks: Tower[]; outlets: Tower[] }
...
export class ProjectileCircuitController {
  ...
  sync() {
    ...
    this.circuits.clear(); this.activeEdges.clear();
    ...
    for (const first of graph.keys()) {
      ...
      const circuit = { banks: members.filter(t => towerFormType(t) === "0"),
        outlets: members.filter(t => towerFormType(t) === "1") };
      if (!circuit.banks.length || !circuit.outlets.length) continue;
  ...
  capture(projectile: Projectile) {
    ...
    if (!source?.inPlay || projectile.type === "chevron" || projectile.targetEnemy || projectile.targetBossPart ||
      isLiteralNumberType(towerFormType(source)) || Math.abs(projectile.vx) < .001) return false;
    const circuit = this.circuits.get(source);
    if (!circuit?.outlets.some(t => t.inPlay)) return false;
  ...
  update() {
    ...
    for (const tower of runtime.towers) {
      ...
      while (bank.remaining > 0 && bank.nextAt <= runtime.battleTime) {
        ...
        const count = Math.min(bank.remaining, Math.max(1, numberTowerValue(outlet)));
        const shots = bank.shots.splice(0, count);
        bank.remaining -= shots.length;
        if (!shots.length) { bank.remaining = 0; break; }
        for (const shot of shots) runtime.emit(shot, outlet);
