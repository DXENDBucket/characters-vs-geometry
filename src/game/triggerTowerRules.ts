import type { TowerActionDataListener as TowerActionListener } from "./towerActions";
import { towerBehaviorType } from "./towerIdentity";
import type { TowerShockAction } from "./battleActions";
import { BOARD_HEIGHT, BOARD_WIDTH, BOARD_X, BOARD_Y, CELL_HEIGHT, CELL_WIDTH } from "../config";
import type { TriggerTowerPresentation } from "./triggerTowerPresentation";
...
export interface TriggerTowerRuntime {
  ...
  scheduleBattleAction: (delay: number, action: TowerShockAction) => void;
  presentation: TriggerTowerPresentation;
  enemies: Enemy[];
  boss: CubeBoss | null;
  battleTime: number;
  gameOver: boolean;
...
export function triggerShockTower(runtime: TriggerTowerRuntime, tower: Tower) {
  ...
  if (triggerDebuff) {
    if (triggerDebuff === "frozen") {
      runtime.presentation.freeze(x, y, rangeX);
    } else if (triggerDebuff === "reversed") {
      runtime.presentation.reversal(x, y, rangeX);
    } else {
      runtime.presentation.shock(area.x, area.y, area.rangeX, area.rangeY, damageType);
    }
...
export function triggerTrapTower(runtime: TriggerTowerRuntime, tower: Tower, target: Enemy | CubeBoss | "boss") {
  ...
  runtime.removeTower(tower);
  runtime.presentation.trap(x, y, damageType);
  if (target === "boss") {
