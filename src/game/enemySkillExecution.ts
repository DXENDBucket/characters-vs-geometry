...
const executors = new WeakMap<EnemySkillRuntime, ReturnType<typeof createSkillExecutor>>();

export function updateEnemySkills(runtime: EnemySkillRuntime, seconds: number, time: number) {
  let executor = executors.get(runtime);
  if (!executor) {
    executor = createSkillExecutor();
    executors.set(runtime, executor);
  }
  executor(runtime, seconds, time);
}
...
function createSkillExecutor() {
  ...
  function updateEnemySkills(runtime: EnemySkillRuntime, seconds: number, time: number) {
    ...
    try {
      ...
      if (activeHeartEnemies.length > 0) {
        updateHeartLeads(runtime.presentation, runtime.enemies, activeHeartEnemies, seconds, time);
      }
    } finally {
      activeHeartEnemies.length = 0;
    }
  ...
  function updateHexHeal(enemy: Enemy, state: SkillState, seconds: number, time: number, runtime: EnemySkillRuntime) {
    if (chargeEnemySkill("heal", state, seconds, time)) tryUseHexHeal(runtime.presentation, runtime.enemies, enemy, state);
  }
  ...
  function updateAngelWings(enemy: Enemy, state: SkillState, seconds: number, time: number, runtime: EnemySkillRuntime) {
    if (chargeEnemySkill("wings", state, seconds, time)) {
      triggerAngelWings(runtime.presentation, runtime.enemies, enemy, time, state);
    }
  }
  ...
  function updateArchangelAscension(enemy: Enemy, state: SkillState, seconds: number, time: number, runtime: EnemySkillRuntime) {
    if (chargeEnemySkill("ascension", state, seconds, time)) {
      triggerArchangelAscension(runtime.presentation, runtime.enemies, enemy, time, state);
    }
  }
  ...
  function updateHeartLead(enemy: Enemy, state: SkillState, seconds: number, time: number, runtime: EnemySkillRuntime) {
    ...
    tryUseHeartLead(runtime.presentation, runtime.enemies, enemy, state);
  }

  function updateHeartLeads(presentation: EnemySkillPresentation, enemies: Enemy[], activeHeartEnemies: Enemy[], seconds: number, time: number) {
    ...
    try {
      ...
      for (const plan of leadPlans) {
        ...
        for (const target of plan.targets) {
          ...
          presentation.position(target);
          presentation.shift(target.x, previousY, target.x, target.y);
        }
      }
    } finally {
      readyCasters.length = 0;
  ...
  function tryUseHexHeal(presentation: EnemySkillPresentation, enemies: Enemy[], healer: Enemy, skill: SkillState) {
    ...
    for (const member of target.healthPool?.members ?? [target]) presentation.scale(member);
    presentation.heal(target.x, target.y);
  }
  ...
  function tryUseHeartLead(presentation: EnemySkillPresentation, enemies: Enemy[], caster: Enemy, skill: SkillState) {
    const originalPositions = enemyPositionMap(enemies);
...
export function triggerAngelWings(
  ...
  triggerWingsEffect(presentation, enemies, caster, time, skill, {
...
function triggerWingsEffect(
  ...
  for (const target of enemies) {
    ...
    presentation.wings(recipient.x, recipient.y);
  }
}
