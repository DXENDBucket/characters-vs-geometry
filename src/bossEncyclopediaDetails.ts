...
export function bossDetailSections(icon: BossIcon, level: number): DetailSection[] {
  ...
  if (icon === "del") {
    passive("75% 生命机制", "75% HP event", l("首次降至 75% 生命：中间三行红色预警 3 秒，本体金色故障闪动并无敌。随后以速度 50 向左完全离场，再从右侧向左回到原位停止，全程不触发底线且保持无敌。移动时碰撞箱经过的格子擦除并封禁 40 秒。限时封格的 × 按剩余时间从原尺寸缩至 50%，永久封格不缩小。",
      "First reaching 75% HP: warns the middle three lanes in red for 3s; DEL glitches gold and becomes invincible. Moves left at speed 50 until fully off-board, then returns from the right to its original position. Remains invincible and cannot breach the base throughout. Cells touched by its moving hitbox are erased and sealed for 40s. Timed seal marks shrink from full size to 50%; permanent marks do not shrink."), [battlefieldRange]);
