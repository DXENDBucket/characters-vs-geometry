...
function towerDescription(id: CardId) {
  ...
  const descriptions: Record<CardId, string> = {
    ...
    "0": zh ? "弹幕蓄存器。自动接收和转发普通弹幕，容量为 128 × 有效等级。空格和无关塔只是通路，按可达接收节点轮流均分，不按分支数分配。双向连接允许回流；不复制，每个战斗步最多转交到下一个实际节点。堵塞或断线时保留库存，摧毁后库存消失。不处理追踪弹、激光、迫击炮、技能或生产。原伤害、属性、多判次数和剩余射程不变。升级保留库存，支持自动升级及提取池多级部署。临时等级消失时保留超额库存，但满仓期间不再收弹。"
      : "Projectile buffer. Automatically receives and forwards ordinary shots; capacity is 128 per effective level. Empty cells and unrelated towers are transparent routes. Rotates equally among reachable receivers, not branches. Bidirectional links allow backflow. No duplication; at most one delivery to the next actual node per simulation tick. Blocked or disconnected stock stays local; destruction loses it. Excludes homing shots, lasers, mortars, skills and production. Preserves damage, effects, hit counts and range. Upgrades retain stock; supports auto-upgrades and extraction batches. Losing temporary levels retains excess stock but blocks input while full.",
    ...
    "+": zh ? "合束加工器。容量和每秒处理上限均为 25 × 有效等级。固定将 5 颗兼容弹幕合为 1 颗：来源、种类、单次伤害、附带效果、速度和剩余射程须一致。成品优先尝试向下游输出，堵塞时留在本地，也可以再次参与合成。保留原有每一次独立伤害判定，不改变破甲线，不增加伤害。自身不向场上发射，由数字出口发射。"
      : "Bundling processor. Capacity and processing rate are 25 shots per effective level and 25 shots/s per effective level. Combines exactly five compatible shots sharing source, kind, per-hit damage, effects, velocity and remaining range. Finished shots try downstream output first; blocked results stay local and can be bundled again. Preserves every individual damage and armor judgment without creating damage. Numeric outlets, not the processor, fire into the battlefield.",
    ...
    "1": zh ? "弹幕出口，只进不出。接收相邻管道送来的弹幕，必须存够与自身数字相同的颗数后才一起向场上发射，不向其他管道节点转发。按自身朝向发射，保留散射角度、伤害、属性、多判次数和剩余射程。升级只增加数字和所需蓄存数量，不提升面板，不额外生成伤害；不参与自动升级。"
      : "Input-only projectile outlet. Collects adjacent pipeline deliveries and fires exactly its displayed number together only when the batch is full. Never forwards to other nodes. Uses its facing and retains spread, damage, effects, hit counts and remaining range. Upgrades increase the number and required batch size, not stats or damage. No auto-upgrades.",
