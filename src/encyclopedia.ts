...
function towerDescription(id: CardId) {
  ...
  const descriptions: Record<CardId, string> = {
    ...
    "*": zh ? "法术护盾出口，只进不出。保护自身为中心、缺四角的 5×5 范围内的友方塔，包括自己，受 & 拓扑影响。在目标法抗结算后，每抵消 1 点法术伤害消耗 3 点库存伤害；储备不足时抵消部分伤害。多个出口按部署顺序补足减免，不重复消耗。仅成功抵消时，在受保护塔上显示短暂蓝色光环护盾。物理伤害和真实伤害不受影响。多判按剩余总伤害计入储备，不执行库存原本的攻击或附带效果，不接收无伤害行动。缓存为 128 × 有效等级，升级只增加容量，保留库存。"
    ...
    "()": shellDescription,
    "[]": shellDescription,
    "!": zh ? "持续攻击附着卡。放在已有塔上，永久允许该塔按原攻速和连射规则执行无需锁定目标的常规攻击，即使本行或射程内没有敌怪。适用于平射弹幕、激光、范围波；无需点选目标的技能满技力自动释放；需要选位置或方向的技能仍手动释放。不绕过小 x、追踪迫击炮、斩击等必须有目标的条件。附着后显示金色 !，重复使用不叠加，升级和移动保留效果。生效后本卡冷却缩短为 30 秒 / 自身有效等级。"
      : "Continuous-fire attachment. Apply to an existing tower to permanently allow free-aim regular attacks at their normal cadence and volley count, even with no enemies in the lane or range. Supports ordinary projectiles, lasers and area waves. Automatically activates ready skills that need no targeting; skills requiring a position or direction remain manual. Does not bypass target requirements for homing shots, predictive mortars or targeted slashes. Shows a gold ! marker, does not stack, and persists through upgrades and movement. After resolving, this card's cooldown becomes 30s / its effective level.",
    "0": zh ? "管道蓄存器。自动接收和转发弹幕或行动效果，容量为 128 × 有效等级。支持激光、追踪弹、迫击炮、生产、治疗、技能和一次性效果。源塔照常支付技力、攻击冷却和自伤；一次性塔照常消失，只把效果存入管道。保留输入时等级、攻击力、属性及多判信息，不复制行动。空格和无关塔是透明通路，按可达接收节点轮流均分；双向连接允许回流，每步最多转交一次。堵塞或断线保留库存，摧毁后库存消失。升级提升容量，支持自动升级；临时等级消失时不删除超额库存。"
      : "Pipeline buffer with capacity 128 per effective level. Automatically forwards projectiles or action payloads, including lasers, homing shots, mortars, production, healing, skills and one-shot effects. Sources still pay SP, attack cooldowns and self-damage; consumed towers disappear while their effects enter the pipe. Snapshots input level, attack, type and hit counts without duplication. Transparent routes distribute equally among reachable receivers, with backflow and at most one delivery per tick. Disconnection retains stock; destruction loses it. Upgrades and auto-upgrades expand capacity; losing temporary levels retains excess stock.",
    "-": zh ? "消去器，只进不出。拥有与 0 相同的 128 × 有效等级本地缓存，不会向管道转发。每 0.1 秒最多使用一颗已收到的弹幕，拦截半径 2.6 格内的敌方平射弹或迫击炮弹。每消去 1 点敌方伤害消耗 5 点我方伤害，双方余量保留，多判逐次扣除。未完全消去的敌弹仍会命中或爆炸。多个减号可共同拦截；激光和非弹体技能不受影响。升级保留库存，临时等级消失不删除超额库存。"
      : "Input-only interceptor with a local buffer of 128 shots per effective level, like 0. Never forwards ammunition. Every 0.1s uses at most one delivered shot against enemy bullets or mortars within 2.6 cells. Spends 5 friendly damage per 1 enemy damage canceled, preserving both remainders and individual judgments. Surviving enemy shots still hit or explode. Multiple interceptors can cooperate; lasers and non-projectile skills are unaffected. Upgrades retain stock, including excess stock after temporary levels expire.",
    "+": zh ? "治疗出口，只进不出。将收到的伤害以 5:1 转换为治疗，治疗自身为中心、缺四角的 5×5 范围内所有友方塔，范围同小 e，受 & 的拓扑交换影响，不显示光环。多判按剩余总伤害换算；消耗库存后不再执行原攻击或附带效果。范围内没有受伤塔时保留库存。缓存与 - 相同，为 128 × 有效等级；处理速度固定为每秒 25 份。升级只提升缓存容量，保留库存；临时等级消失不删除超额库存。不接收生产等无伤害行动。"
      : "Input-only healing outlet. Converts incoming damage to healing at 5:1 for every friendly tower in a centered 5x5 area without the four corners, including itself. Matches small e's range, respects & topology and has no aura outline. Counts remaining total multi-hit damage; consumed payloads do not execute their original attacks or effects. Retains stock when nobody in range is injured. Like -, capacity is 128 per effective level; consumption stays at 25 payloads/s. Upgrades increase capacity only and retain stock, including excess stock after temporary levels expire. Rejects non-damaging actions such as production.",
    ...
    "=": zh ? "特殊型边连接件，不占格且不受攻击。点击切换双向 =、单向 >、反向 <、关闭 ≠；纵向旋转方向。通路可经过空格和无关塔，按可达接收节点均分，同一节点有多条路径也只算一份。遇到 0、+、1、-、*、/ 必须交给该节点，满仓不能穿过。每条连接每秒流速为 25 × 等级，最多积攒一秒额度；整条路径成功才扣流量，无缓存。源塔基础费用须不超过 999；无可用通路时正常发射。选中 = 叠放升级，可挂自动升级；橡皮擦移除。"
      : "Special edge connector; no tile occupancy or attackable body. Click cycles bidirectional, forward, backward and closed modes; vertical directions rotate. Routes pass through empty cells and unrelated towers, distributing equally among reachable receivers regardless of path count. A 0, +, 1, -, * or / stops transit, even when full. Each link refills 25 shots/s per level with one second of credit; charge the whole path only on successful delivery. No storage. Sources must cost at most 999 and fire normally when no route is available. Stack = to upgrade, mark for auto-upgrade, or erase the edge.",
    "1": zh ? "管道出口，只进不出。收到与自身数字相同数量的弹幕或行动后，一起发射或执行。使用出口位置和朝向，保留输入时的等级、攻击力、属性、多判及弹幕剩余射程；有目标的技能保留选定目标。灰烬效果不消耗出口，因为源塔已经消失。只执行实际收到的行动，不自动生成额外攻击或技能。升级仅增加数字和等待数量，不提高输出等级或伤害；不参与自动升级。"
      : "Input-only pipeline outlet. Waits for its displayed number of projectiles or actions, then fires or executes the batch together. Uses the outlet's position and facing while preserving input level, attack, type, hit counts and remaining projectile range; targeted skills keep selected targets. Consumed-tower effects do not consume the outlet: the source has already disappeared. Executes only delivered actions, with no free attacks or skills. Upgrades change grouping only, not output level or damage. No auto-upgrades.",
    ...
    a: zh ? "短程物理射手。机制类似 A，但只攻击自身和前方 4 格内的目标。" : "Short-range physical shooter. Similar to A, but only attacks within itself plus 4 tiles ahead.",
    ...
    O: zh ? "抗法防御塔。机制和 D 类似，拥有中等护甲和高法术抗性。" : "Magic-resistant defender. Similar to D, with moderate armor and high magic resistance.",
    ...
    E: zh ? "三连物理射手。前方扇形区域内有可攻击目标时开火；向前平射，并向上/下各偏转 10 度发射一发。转向时判定扇形与弹幕一起反向。" : "Triple physical shooter. Fires when an attackable target is inside its forward fan: one straight shot plus two shots at +/-10 degrees. Reversing mirrors both the targeting fan and projectiles.",
    ...
    w: zh ? "巡空防御塔。数值和 B 相同；初始 8 技力，每秒回复 1 技力，上限 10。满技力时外框闪烁；点击消耗 10 技力，获得 10 秒飞行和光环。飞行期间不阻挡地面敌人，但可以阻挡普通飞行敌人；高空飞行不会被阻挡。技能结束后才重新回技，升级会重置技力。" : "Air patrol defender. Same baseline stats as B; starts at 8 SP, gains 1 SP/s up to 10. At full SP its border flashes; click to spend 10 SP and gain 10s Flying with a halo. While flying it no longer blocks ground enemies, but can block regular Flying enemies; High Flight is never blocked. SP regeneration resumes only after the skill ends, and upgrades reset SP.",
    ...
    j: zh ? "汇聚塔。初始 0 技力，每秒回复 1，上限 10；满技力后点击消耗 10，持续 10 秒，期间不回技。将自身上、下各一行同列格内的敌我双方子弹移至自身所在行，每转移一颗自损 100 真实伤害。保留子弹阵营、伤害、多判次数、速度和锁定目标；敌方子弹仍会伤害我方塔。子弹每次转移后需间隔 0.1 秒才能再次转移，允许多个小 j 反复抢夺；同一帧多个小 j 拉同一颗子弹时全部不生效、不自损。不影响激光和迫击抛射体。" : "Gatherer. Starts at 0 SP, recovers 1 SP/s up to 10. Click to spend 10 SP for 10s; recovery pauses while active. Moves allied and enemy bullets in the same-column cells immediately above and below into its lane, taking 100 true self-damage per bullet. Preserves allegiance, damage, hit count, velocity and locked targets; enemy bullets still damage allied towers. Each bullet has a 0.1s interval between transfers; gatherers can steal it repeatedly. Multiple gatherers pulling the same bullet in one frame cancel all those pulls without self-damage. Lasers and mortar projectiles are unaffected.",
...
export function towerUpgradeText(id: CardId) {
  ...
  if (id === "1") return isZh() ? "手动升级数字 +1，须存满对应数量才整批发射；不增加伤害，不参与自动升级。"
    : "Manual number +1; must fill the required batch before firing it together. No extra damage or auto-upgrades.";
  if (id === "=") return isZh() ? "每级增加每秒 25 颗流速，支持自动升级。" : "Each level adds 25 shots/s flow. Supports auto-upgrades.";
  if (id === "*" || id === "/") return isZh() ? "每有效等级增加 128 份缓存，升级保留库存；3:1 的伤害抵消比例、伤害类型和保护范围不变。" : "Each effective level adds 128 capacity, retaining stock. The 3:1 absorption ratio, damage type and protection range stay unchanged.";
  if (id === "+") return isZh() ? "与 - 相同：每有效等级增加 128 份缓存，升级保留库存；治疗转化比例和处理速度不变。" : "Like -: each effective level adds 128 capacity, retaining stock. Healing conversion and processing speed stay unchanged.";
  if (id === "-") return isZh() ? "与 0 相同：每有效等级增加 128 颗缓存，升级保留库存。" : "Like 0: each effective level adds 128 capacity, retaining stock on upgrade.";
  if (id === "@") {
...
export function mechanicEncyclopediaEntries(): EncyclopediaEntry[] {
  ...
  const entries: Array<{
    ...
    },
    {
      ...
      linesZh: ["效果：永久允许无目标的常规攻击", "无目标技能自动释放；需选位置或方向的技能仍手动，不绕过攻击锁定条件"],
      linesEn: ["Effect: permanently enables free-aim regular attacks", "Auto-casts untargeted skills; position/direction selection stays manual, attack locks still apply"],
