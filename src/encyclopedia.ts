...
function towerDescription(id: CardId) {
  ...
  const descriptions: Record<CardId, string> = {
    ...
    "+": zh ? "合束出口。通过 = 连接蓄存器 0，点击 0 后每批接收最多 5 颗弹幕。同来源、同种类、伤害与附带效果、速度方向和剩余射程一致的弹幕合为一颗，其余分别射出。保留每一次独立伤害判定，不合并单次伤害，不提高破甲能力。按自身朝向发射，合束弹体更大，不额外生成伤害。"
      : "Bundling outlet. Connect to a zero bank with = and click zero to discharge up to five shots per batch. Shots with matching source, kind, damage, effects, velocity and remaining range merge into one body; others fire separately. Preserves individual judgments and armor checks instead of adding damage into a single hit. Uses its own facing. Bundles appear larger without creating damage.",
    ...
    "=": zh ? "特殊型边连接件。部署在内部网格线上，只连接该边两侧格子的塔，不占普通格子；没有生命、阻挡或受击，不参与治疗、镜像、升级和移位器移动。橡皮擦点击该边移除。连接基础费用不超过 999 的源塔及加工器 +、-。网络需有蓄存器 0 和数字出口、合束出口或消去器才会截存弹幕。浅绿色表示网络完整。不再学习或模仿任何塔的行动。"
      : "Special edge connector. Place on an internal grid edge to link its two adjacent cells without occupying either cell. No HP, blocking, damage, healing, mirroring, upgrades or shifter movement. Erase by clicking the edge. Connects sources costing at most 999 and the + / - processors. Capture needs a zero bank and a numeric outlet, bundler or subtractor. Mint links indicate a complete circuit. No learned actions or imitation.",
