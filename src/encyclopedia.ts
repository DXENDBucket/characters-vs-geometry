...
function towerDescription(id: CardId) {
  ...
  const descriptions: Record<CardId, string> = {
    ...
    "0": zh ? "弹幕蓄存器。通过边连接件逐节点接收并自动转发普通弹幕，容量为 128 × 有效等级。所有可用方向轮流分配，双向连接允许回流；不复制，每个战斗步最多经过一条连接。通路堵塞或断线时保留库存，摧毁后库存消失。不处理追踪弹、激光、迫击炮、技能或生产。原伤害、属性、多判次数和剩余射程不变。升级保留库存，支持自动升级及提取池多级部署。临时等级消失时保留超额库存，但满仓期间不再收弹。"
      : "Projectile buffer. Receives ordinary shots through edge links and automatically forwards them node by node. Holds 128 shots per effective level. Rotates between all available routes, including backflow through bidirectional links. No duplication; at most one link per simulation tick. Blocked or disconnected stock stays local; destruction loses it. Excludes homing shots, lasers, mortars, skills and production. Preserves damage, effects, hit counts and remaining range. Upgrades retain stock; supports auto-upgrades and extraction batches. Losing temporary levels retains excess stock but blocks further input until space is available.",
    ...
    "=": zh ? "特殊型边连接件。部署在内部网格线上，不占普通格子且不受攻击。点击依次切换双向 =、单向 >、反向 <、关闭 ≠；纵向按旋转后的方向传输。自身没有缓存，每秒流速为 25 × 等级，最多积攒一秒传输额度。源塔没有可用输入通路时正常发射。源塔基础费用须不超过 999，加工器 +、- 不受此限制。选中 = 卡叠放可升级，也可挂自动升级；橡皮擦点击边移除。"
      : "Special internal-edge connector; occupies no tile and cannot be attacked. Click to cycle bidirectional =, forward >, backward < and closed modes; vertical links rotate these directions. Has no ammunition storage. Flow refills at 25 shots/s per level, with up to one second of credit. Sources fire normally when no input route is available. Sources must cost at most 999; + and - are exempt. Stack the selected = card to upgrade, or mark for auto-upgrade. Erase by clicking the edge.",
