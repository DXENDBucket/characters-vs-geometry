...
function towerDescription(id: CardId) {
  ...
  const descriptions: Record<CardId, string> = {
    "+": zh ? "等式加法连接器。A+E 中，A 的行动也为数字塔记忆中的 E 计数，反之亦然，不会让实体源塔额外行动。数字相加时，加号自身按整组数字之和模仿：3+5 中 + 按数字 8 行动，3+5+2 中两个 + 都按 10 行动；原数字仍各自正常生效。数字与普通塔不能直接相加。可混用等号，共享整个体系的源塔记忆，但等号两侧不合并求和。只连接基础费用不超过 999 的操作数，相邻运算符或高价塔中断连接。"
      : "Equation addition connector. In A+E, either source advances both learned counters without extra actions by real towers. With numeric operands, each + imitates at the entire plus group's sum: 3+5 gives + the behavior of 8; both pluses in 3+5+2 act as 10. Original numbers still act independently. Numbers cannot be added to ordinary towers. Equals shares source memories throughout the equation but does not combine sums. Operands must cost at most 999; adjacent operators and expensive towers break connections.",
