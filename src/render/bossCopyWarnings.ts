  ...
  for (const spawn of pending) {
    ...
    if (!visual) {
      ...
      const countdown = scene.add.text(0, 0, "", {
        fontFamily: "monospace", fontSize: "26px", fontStyle: "700", color: "#ffd75a"
    ...
    visual.frame.fillStyle(palette.gold, 0.06 + pulse * 0.06).fillRect(-w / 2, -h / 2, w, h);
    visual.frame.lineStyle(3, palette.gold, pulse).strokeRect(-w / 2, -h / 2, w, h);
    visual.frame.fillStyle(palette.gold, 0.9).fillRect(-w / 2, h / 2 - 5, w * progress, 5);
