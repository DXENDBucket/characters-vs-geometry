...
export interface BoardToolHint {
  x: number;
  y: number;
  shape: "tower" | "parenthesis" | "squareBracket" | "edge" | "cell";
  action: "select" | "deselect" | "autoOn" | "autoOff" | "erase" | "invalid";
}

export function drawTowerSelection(graphics: Phaser.GameObjects.Graphics, type?: CardId, color = palette.magic) {
  if (isTowerShellType(type)) drawTowerShellBorder(graphics, color, 2, 1, type);
...
export class BoardToolPreview {
  ...
  show(hints: readonly BoardToolHint[]) {
    ...
    for (const hint of hints) {
      ...
      this.body.add(mark);
      if (hint.shape === "parenthesis") drawTowerShellBorder(mark, color, 3, 1);
      else if (hint.shape === "squareBracket") drawTowerShellBorder(mark, color, 3, 1, "[]");
      ...
      else drawTowerSelection(mark, undefined, color);
      const symbol = hint.action === "erase" || hint.action === "invalid" ? "×"
        : hint.action === "deselect" || hint.action === "autoOff" ? "−" : hint.action === "autoOn" ? "↑" : "+";
      const x = hint.shape === "parenthesis" || hint.shape === "squareBracket" ? 32 : hint.shape === "edge" ? 0 : 19;
